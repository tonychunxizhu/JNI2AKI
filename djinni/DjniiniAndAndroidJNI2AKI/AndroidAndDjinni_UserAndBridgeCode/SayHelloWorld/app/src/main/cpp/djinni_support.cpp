...
/*static*/
    ...
    static void walk_through_backtrace(JNIEnv *env,
        ...
        LocalRef<jobjectArray> elements{
                (jobjectArray) env->CallObjectMethod(exception, throwable_getStackTrace)};
        if (elements != nullptr) {
        ...
        LocalRef<jthrowable> cause{
                (jthrowable) env->CallObjectMethod(exception, throwable_get_cause)};
        if (cause != nullptr) {
    ...
    void jni_exception::capture_backtrace() {
        ...
        jmethodID throwable_get_cause = jniGetMethodID(throwable_class.get(), "getCause",
                                                       "()Ljava/lang/Throwable;");
        jmethodID throwable_getStackTrace = jniGetMethodID(throwable_class.get(), "getStackTrace",
                                                           "()[Ljava/lang/StackTraceElement;");
        jmethodID throwable_toString = jniGetMethodID(throwable_class.get(), "toString",
                                                      "()Ljava/lang/String;");
        GlobalRef<jclass> ste_class = jniFindClass("java/lang/StackTraceElement");
        jmethodID ste_toString = jniGetMethodID(ste_class.get(), "toString",
                                                "()Ljava/lang/String;");
 ...
 */
    ...
    JniEnum::JniEnum(const std::string &name)
            : m_clazz{jniFindClass(name.c_str())},
              m_staticmethValues{
                      jniGetStaticMethodID(m_clazz.get(), "values", ("()[L" + name + ";").c_str())},
    ...
    LocalRef<jobject> JniEnum::create(JNIEnv *env, jint value) const {
        LocalRef<jobject> values(env,
                                 env->CallStaticObjectMethod(m_clazz.get(), m_staticmethValues));
        jniExceptionCheck(env);
    ...
    LocalRef<jobject> JniFlags::create(JNIEnv *env, unsigned flags, int bits) const {
        auto j = LocalRef<jobject>(env, env->CallStaticObjectMethod(m_clazz.get(), m_methNoneOf,
                                                                    enumClass()));
        jniExceptionCheck(env);
        unsigned mask = 1;
 ...
 */
    ...
    implWStringToUTF16(std::wstring::const_iterator, std::wstring::const_iterator) {
    ...
    implWStringToUTF16<2>(std::wstring::const_iterator begin, std::wstring::const_iterator end) {
    ...
    template<>
    inline std::u16string
    implWStringToUTF16<4>(std::wstring::const_iterator begin, std::wstring::const_iterator end) {
 ...
 */
    ...
    private:
        ...
        public:
            ...
            const jmethodID constructor{
                    jniGetMethodID(clazz.get(), "<init>", "(Ljava/lang/Object;)V")};
        ...
        static GlobalRef<jobject> create(JNIEnv *jniEnv, jobject obj) {
            ...
            LocalRef<jobject> weakRef(jniEnv, jniEnv->NewObject(weakRefClass.clazz.get(),
                                                                weakRefClass.constructor, obj));
    ...
    public:
        ...
        jobject lock() const {
            ...
            const JniInfo &weakRefClass = JniClass<JniInfo>::get();
            LocalRef<jobject> javaObj(
                    jniEnv->CallObjectMethod(m_weakRef.get(), weakRefClass.method_get));
